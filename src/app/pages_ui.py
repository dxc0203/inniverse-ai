    ...
    if st.button("批量运行 AI", type="primary", use_container_width=True):
        for idx, data in enumerate(projects_data):
            ...
            with st.spinner(f"AI 正在为 {data['name']} 生成结果..."):
                if data['task_type'] == "从文本生成图像": image_results = generate_image_from_text(final_p, test_mode)
                else: image_results = generate_image_from_image_and_text(processed_imgs, final_p, test_mode)
            if image_results:
...
def render_prompt_management(PROMPTS_DIR):
    ...
    if selected_prompt != "创建新提示":
        try:
            with open(os.path.join(PROMPTS_DIR, selected_prompt), "r", encoding="utf-8") as f: prompt_content = f.read()
        except: pass
...
def render_history(LOGS_DIR):
    ...
    for p in projects:
        ...
        if os.path.exists(os.path.join(p_path, "prompt.txt")):
            with open(os.path.join(p_path, "prompt.txt"), "r", encoding="utf-8") as f: prompt = f.read()
        metadata = {}
        if os.path.exists(os.path.join(p_path, "metadata.json")):
            with open(os.path.join(p_path, "metadata.json"), "r", encoding="utf-8") as f: metadata = json.load(f)
        preview = None
