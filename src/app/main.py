    ...
    with st.expander("快速提示构建器（中文界面，英文提示）", expanded=False):
        ...
        if st.button("生成到提示文本（英文）", key=f"pb_generate_{key_suffix}"):
            ...
            if content_type == "服装/模特相关":
                ...
                if focus_cn_list: cloth_part_en += " Focus on " + ", ".join([focus_map[f] for f in focus_cn_list if f in focus_map]) + "."
            model_part_en = f" Use {aspect_ratio_map.get(aspect_ratio_cn, '')}, {camera_map.get(camera_cn, '')}, {lighting_map.get(lighting_cn, '')}."
            if quality_cn_list: model_part_en += " High quality: " + ", ".join([quality_map[q] for q in quality_cn_list if q in quality_map]) + "."
            final_prompt = (base_en + " " + cloth_part_en + " " + model_part_en + f" Additional: {extra_cn}. Clean composition, no text/watermarks.").strip()
            # Depending on context, we might want to update different session state keys, 
            # but for now let's keep it simple and update 'input_prompt' 
            # or handle it in the calling code.
            return final_prompt
    return None
...
if page == "新项目":
    ...
    if task_type == "从图像 and 文本生成图像":
        newly_uploaded_files = st.file_uploader("上传图片", type=["jpg", "jpeg", "png"], accept_multiple_files=False)
        if newly_uploaded_files: st.session_state["uploaded_files"] = [newly_uploaded_files]

    if task_type == "从图像 and 文本生成图像" and st.session_state["uploaded_files"]:
    ...
    if st.button("运行 AI"):
        ...
        with open(os.path.join(log_dir, "prompt.txt"), "w", encoding="utf-8") as f: f.write(user_prompt)
        with st.spinner("处理中..."):
            if task_type == "从文本生成图像":
                ...
                if image_results:
                    ...
                    for i, img_data in enumerate(image_results):
                        ...
                        with cols[i]: st.image(img_path, caption=f"生成的图像 {i+1}", use_container_width=True)
                    st.success("图像 generation 完成！")
            elif task_type == "从图像 and 文本生成图像":
    ...
    if selected_prompt != "创建新提示":
        ...
        except Exception as e: st.error(f"读取提示文件时出错：{e}")
    ...
    with col1:
        if st.button("保存提示", use_container_width=True):
            if new_prompt_name and prompt_text:
                if not new_prompt_name.endswith(".txt"): new_prompt_name += ".txt"
                try:
                    with open(os.path.join(PROMPTS_DIR, new_prompt_name), "w", encoding="utf-8") as f: f.write(prompt_text)
                    st.success(f"提示 '{new_prompt_name}' 保存成功！")
                    st.session_state["selected_prompt_name"] = new_prompt_name
                    st.rerun()
                except Exception as e: st.error(f"保存提示时出错：{e}")
            else: st.warning("提示名称和内容不能为空。")
    with col2:
        if selected_prompt != "创建新提示":
            if st.button("删除提示", type="primary", use_container_width=True):
                try:
                    ...
                    st.session_state["selected_prompt_name"] = ""
                    st.rerun()
                except Exception as e: st.error(f"删除提示时出错：{e}")
    ...
    if os.path.exists(log_root):
        ...
        if projects:
            ...
            with col1:
                if st.button("重新启动项目", use_container_width=True):
                    ...
                    if os.path.exists(old_prompt_path):
                        with open(old_prompt_path, "r", encoding="utf-8") as f: st.session_state["input_prompt"] = f.read()
            ...
            if os.path.exists(prompt_path):
                with open(prompt_path, "r", encoding="utf-8") as f: st.text_area("使用的提示", f.read(), disabled=True)
            input_dir = os.path.join(project_path, "inputs")
            output_dir = os.path.join(project_path, "outputs")feat: refactor prompt builder into a function and add it to prompt management page
            if os.path.exists(input_dir):
                ...
                if input_files:
                    for f_name in input_files:
                        ...
                        with c1: st.image(img, caption=f"输入：{f_name}", use_container_width=True)
                        ...
                        if os.path.exists(output_dir):
                            for cand in out_candidates:
                                ...
                                if os.path.exists(out_path):
                                    ...
                                    with c2: st.image(out_img, caption=f"输出：{cand}", use_container_width=True)
                                    found_output = True
                                    break
                        if not found_output:
                            with c2: st.info("未找到此输入的输出图像。")
            output_path = os.path.join(project_path, "outputs", "result.txt")
            if os.path.exists(output_path):
                st.subheader("结果")
                with open(output_path, "r", encoding="utf-8") as f: st.markdown(f.read())
        else: st.info("在日志目录中找不到任何项目。")
    else: st.info("未找到日志目录。")
